...
campaigns/<date>-<short-name>/   one directory per campaign
    ...
    iocs.stix.json               STIX 2.1 bundle, generated by tools/build_stix.py
...
- `build_stix.py` — regenerates a campaign's `iocs.stix.json` from its `iocs.csv`,
  preserving the object IDs of indicators that already existed. Run it after editing a
  CSV; the bundle had drifted to 9 of 46 indicators before this existed.
